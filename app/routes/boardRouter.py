...
from app.models import schema, services, models
from app.models import boardService


router = APIRouter()

# 게시글 작성
@router.post("/post/write", response_model=schema.getPost)
def write_post(
    post_data: schema.writePost,
...
# 특정 게시글 조회 (조회수 증가)
@router.get("/post/{post_id}", response_model=schema.getPost)
